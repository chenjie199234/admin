...
import * as configAPI from '../../api/config_browser_toc'
...
const configClient: configAPI.ConfigBrowserClientToC = new configAPI.ConfigBrowserClientToC(host)
...
const node_filter = ref('')
function system_node(node: permissionAPI.NodeInfo):boolean{
...
<template>
	...
	<div v-if="!inited" style="display:flex;flex-direction:column;width:100%;justify-content:center;align-items:center">
		<va-card style="width:400px;margin:10px" color="primary" gradient>
			<va-card-title>Warning</va-card-title>
			<va-card-content>System not initialized.Please initialize below first.</va-card-content>
		</va-card>
		<div style="display:flex;flex-direction:column">
		...
		<div style="display:flex;flex-direction:column;width:300px">
			<div style="display:flex;margin:4px 0">
				...
				<va-popover message="Create New Project" :hover-out-timeout="0" :hover-over-timeout="0" color="primary">
					<va-button v-if="state.user.root" style="width:36px;margin:0 2px" @click="state.set_project('add')">+</va-button>
				</va-popover>
				<va-dropdown v-if="state.project.cur" trigger="hover" style="width:36px;margin:0 2px">
					<template #anchor>
						<va-button>•••</va-button>
					</template>
					<va-dropdown-content>
						...
						<va-popover message="Add Menu" :hover-out-timeout="0" :hover-over-timeout="0" color="primary">
							<va-button v-if="is_root_node_project()&&state.project.nodes[0].admin" style="width:36px;margin:0 3px" @click="state.set_node(state.project.nodes[0],'add')">✿</va-button>
						</va-popover>
					</va-dropdown-content>
				</va-dropdown>
			</div>
			<va-divider style="margin:0" />
			<div style="flex:1;overflow-x:hidden;overflow-y:auto;color: rgb(38, 40, 36); background-color: rgb(236, 240, 241)">
				<va-tree-view :nodes="is_root_node_project()?state.project.nodes[0].children:state.project.nodes" text-by="node_name" track-by="node_id" value-by="node_id">
					<template #icon-toggle="node">
						<div style="height:36px;display:flex;align-items:center">{{ node.expanded?'▲':'▼' }}</div>
					</template>
					<template #content="node">
						<div style="height:36px;display:flex">
							<div :style="{display:'flex','align-items':'center',flex:1,cursor:system_node(node)||node.node_data!=''?'pointer':'default'}" @click="()=>{if(system_node(node)||node.node_data!=''){state.set_page(node)}}">
								<span style="font-size:18px">{{node.node_name}}</span>
							</div>
							<va-dropdown v-if="node_need_button(node)" trigger="hover" style="width:36px" prevent-overflow>
								<template #anchor>
									<va-button>•••</va-button>
								</template>
								<va-dropdown-content>
