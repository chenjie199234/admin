	...
	return func(ctx *web.Context) {
		...
		if strings.HasPrefix(ctx.GetContentType(), "application/json") {
			...
			if e := ctx.ParseForm(); e != nil {
				log.Error(ctx, "[/admin.permission/get_user_permission] parse form failed", log.CError(e))
				ctx.Abort(cerror.ErrReq)
				return
			}
			// req.UserId
			...
			if forms := ctx.GetForms("node_id"); len(forms) > 0 {
				...
				for _, form := range forms {
					if num, e := strconv.ParseUint(form, 10, 32); e != nil {
						log.Error(ctx, "[/admin.permission/get_user_permission] data format wrong", log.String("field", "node_id"))
						ctx.Abort(cerror.ErrReq)
						return
					} else {
...
func _Permission_UpdateUserPermission_WebHandler(handler func(context.Context, *UpdateUserPermissionReq) (*UpdateUserPermissionResp, error)) web.OutsideHandler {
	return func(ctx *web.Context) {
		...
		if strings.HasPrefix(ctx.GetContentType(), "application/json") {
			...
			if e := ctx.ParseForm(); e != nil {
				log.Error(ctx, "[/admin.permission/update_user_permission] parse form failed", log.CError(e))
				ctx.Abort(cerror.ErrReq)
				return
			}
			// req.UserId
			...
			if forms := ctx.GetForms("node_id"); len(forms) > 0 {
				...
				for _, form := range forms {
					if num, e := strconv.ParseUint(form, 10, 32); e != nil {
						log.Error(ctx, "[/admin.permission/update_user_permission] data format wrong", log.String("field", "node_id"))
						ctx.Abort(cerror.ErrReq)
						return
					} else {
						req.NodeId = append(req.NodeId, uint32(num))
			...
			if form := ctx.GetForm("admin"); len(form) != 0 {
				if form == "true" {
					...
					log.Error(ctx, "[/admin.permission/update_user_permission] data format wrong", log.String("field", "admin"))
					ctx.Abort(cerror.ErrReq)
					return
				}
			}
			...
			if form := ctx.GetForm("canread"); len(form) != 0 {
				if form == "true" {
					...
					log.Error(ctx, "[/admin.permission/update_user_permission] data format wrong", log.String("field", "canread"))
					ctx.Abort(cerror.ErrReq)
					return
				}
			}
			...
			if form := ctx.GetForm("canwrite"); len(form) != 0 {
				if form == "true" {
					...
					log.Error(ctx, "[/admin.permission/update_user_permission] data format wrong", log.String("field", "canwrite"))
					ctx.Abort(cerror.ErrReq)
					return
				}
			}
...
func _Permission_UpdateRolePermission_WebHandler(handler func(context.Context, *UpdateRolePermissionReq) (*UpdateRolePermissionResp, error)) web.OutsideHandler {
	return func(ctx *web.Context) {
		...
		if strings.HasPrefix(ctx.GetContentType(), "application/json") {
			...
			if e := ctx.ParseForm(); e != nil {
				log.Error(ctx, "[/admin.permission/update_role_permission] parse form failed", log.CError(e))
				ctx.Abort(cerror.ErrReq)
				return
			}
			// req.ProjectId
			if forms := ctx.GetForms("project_id"); len(forms) > 0 {
				...
				for _, form := range forms {
					if num, e := strconv.ParseUint(form, 10, 32); e != nil {
						log.Error(ctx, "[/admin.permission/update_role_permission] data format wrong", log.String("field", "project_id"))
						ctx.Abort(cerror.ErrReq)
						return
					} else {
			...
			if forms := ctx.GetForms("node_id"); len(forms) > 0 {
				...
				for _, form := range forms {
					if num, e := strconv.ParseUint(form, 10, 32); e != nil {
						log.Error(ctx, "[/admin.permission/update_role_permission] data format wrong", log.String("field", "node_id"))
						ctx.Abort(cerror.ErrReq)
						return
					} else {
						req.NodeId = append(req.NodeId, uint32(num))
			...
			if form := ctx.GetForm("admin"); len(form) != 0 {
				if form == "true" {
					...
					log.Error(ctx, "[/admin.permission/update_role_permission] data format wrong", log.String("field", "admin"))
					ctx.Abort(cerror.ErrReq)
					return
				}
			}
			...
			if form := ctx.GetForm("canread"); len(form) != 0 {
				if form == "true" {
					...
					log.Error(ctx, "[/admin.permission/update_role_permission] data format wrong", log.String("field", "canread"))
					ctx.Abort(cerror.ErrReq)
					return
				}
			}
			...
			if form := ctx.GetForm("canwrite"); len(form) != 0 {
				if form == "true" {
					...
					log.Error(ctx, "[/admin.permission/update_role_permission] data format wrong", log.String("field", "canwrite"))
					ctx.Abort(cerror.ErrReq)
					return
				}
			}
...
func _Permission_AddNode_WebHandler(handler func(context.Context, *AddNodeReq) (*AddNodeResp, error)) web.OutsideHandler {
	return func(ctx *web.Context) {
		...
		if strings.HasPrefix(ctx.GetContentType(), "application/json") {
			...
			if e := ctx.ParseForm(); e != nil {
				log.Error(ctx, "[/admin.permission/add_node] parse form failed", log.CError(e))
				ctx.Abort(cerror.ErrReq)
				return
			}
			// req.PnodeId
			if forms := ctx.GetForms("pnode_id"); len(forms) > 0 {
				...
				for _, form := range forms {
					if num, e := strconv.ParseUint(form, 10, 32); e != nil {
						log.Error(ctx, "[/admin.permission/add_node] data format wrong", log.String("field", "pnode_id"))
						ctx.Abort(cerror.ErrReq)
						return
					} else {
...
func _Permission_UpdateNode_WebHandler(handler func(context.Context, *UpdateNodeReq) (*UpdateNodeResp, error)) web.OutsideHandler {
	return func(ctx *web.Context) {
		...
		if strings.HasPrefix(ctx.GetContentType(), "application/json") {
			...
			if e := ctx.ParseForm(); e != nil {
				log.Error(ctx, "[/admin.permission/update_node] parse form failed", log.CError(e))
				ctx.Abort(cerror.ErrReq)
				return
			}
			// req.NodeId
			if forms := ctx.GetForms("node_id"); len(forms) > 0 {
				...
				for _, form := range forms {
					if num, e := strconv.ParseUint(form, 10, 32); e != nil {
						log.Error(ctx, "[/admin.permission/update_node] data format wrong", log.String("field", "node_id"))
						ctx.Abort(cerror.ErrReq)
						return
					} else {
						req.NodeId = append(req.NodeId, uint32(num))
...
func _Permission_MoveNode_WebHandler(handler func(context.Context, *MoveNodeReq) (*MoveNodeResp, error)) web.OutsideHandler {
	return func(ctx *web.Context) {
		...
		if strings.HasPrefix(ctx.GetContentType(), "application/json") {
			...
			if e := ctx.ParseForm(); e != nil {
				log.Error(ctx, "[/admin.permission/move_node] parse form failed", log.CError(e))
				ctx.Abort(cerror.ErrReq)
				return
			}
			// req.NodeId
			if forms := ctx.GetForms("node_id"); len(forms) > 0 {
				...
				for _, form := range forms {
					if num, e := strconv.ParseUint(form, 10, 32); e != nil {
						log.Error(ctx, "[/admin.permission/move_node] data format wrong", log.String("field", "node_id"))
						ctx.Abort(cerror.ErrReq)
						return
					} else {
						req.NodeId = append(req.NodeId, uint32(num))
			...
			if forms := ctx.GetForms("pnode_id"); len(forms) > 0 {
				...
				for _, form := range forms {
					if num, e := strconv.ParseUint(form, 10, 32); e != nil {
						log.Error(ctx, "[/admin.permission/move_node] data format wrong", log.String("field", "pnode_id"))
						ctx.Abort(cerror.ErrReq)
						return
					} else {
						req.PnodeId = append(req.PnodeId, uint32(num))
...
func _Permission_DelNode_WebHandler(handler func(context.Context, *DelNodeReq) (*DelNodeResp, error)) web.OutsideHandler {
	return func(ctx *web.Context) {
		...
		if strings.HasPrefix(ctx.GetContentType(), "application/json") {
			...
			if e := ctx.ParseForm(); e != nil {
				log.Error(ctx, "[/admin.permission/del_node] parse form failed", log.CError(e))
				ctx.Abort(cerror.ErrReq)
				return
			}
			// req.NodeId
			if forms := ctx.GetForms("node_id"); len(forms) > 0 {
				...
				for _, form := range forms {
					if num, e := strconv.ParseUint(form, 10, 32); e != nil {
						log.Error(ctx, "[/admin.permission/del_node] data format wrong", log.String("field", "node_id"))
						ctx.Abort(cerror.ErrReq)
						return
					} else {
						req.NodeId = append(req.NodeId, uint32(num))
...
func _Permission_ListUserNode_WebHandler(handler func(context.Context, *ListUserNodeReq) (*ListUserNodeResp, error)) web.OutsideHandler {
	return func(ctx *web.Context) {
		...
		if strings.HasPrefix(ctx.GetContentType(), "application/json") {
			...
			if e := ctx.ParseForm(); e != nil {
				log.Error(ctx, "[/admin.permission/list_user_node] parse form failed", log.CError(e))
				ctx.Abort(cerror.ErrReq)
				return
			}
			// req.ProjectId
			if forms := ctx.GetForms("project_id"); len(forms) > 0 {
				...
				for _, form := range forms {
					if num, e := strconv.ParseUint(form, 10, 32); e != nil {
						log.Error(ctx, "[/admin.permission/list_user_node] data format wrong", log.String("field", "project_id"))
						ctx.Abort(cerror.ErrReq)
						return
					} else {
						req.ProjectId = append(req.ProjectId, uint32(num))
			...
			if form := ctx.GetForm("need_user_role_node"); len(form) != 0 {
				if form == "true" {
					...
					log.Error(ctx, "[/admin.permission/list_user_node] data format wrong", log.String("field", "need_user_role_node"))
					ctx.Abort(cerror.ErrReq)
					return
				}
			}
...
func _Permission_ListRoleNode_WebHandler(handler func(context.Context, *ListRoleNodeReq) (*ListRoleNodeResp, error)) web.OutsideHandler {
	return func(ctx *web.Context) {
		...
		if strings.HasPrefix(ctx.GetContentType(), "application/json") {
			...
			if e := ctx.ParseForm(); e != nil {
				log.Error(ctx, "[/admin.permission/list_role_node] parse form failed", log.CError(e))
				ctx.Abort(cerror.ErrReq)
				return
			}
			// req.ProjectId
			if forms := ctx.GetForms("project_id"); len(forms) > 0 {
				...
				for _, form := range forms {
					if num, e := strconv.ParseUint(form, 10, 32); e != nil {
						log.Error(ctx, "[/admin.permission/list_role_node] data format wrong", log.String("field", "project_id"))
						ctx.Abort(cerror.ErrReq)
						return
					} else {
						req.ProjectId = append(req.ProjectId, uint32(num))
...
func _Permission_ListProjectNode_WebHandler(handler func(context.Context, *ListProjectNodeReq) (*ListProjectNodeResp, error)) web.OutsideHandler {
	return func(ctx *web.Context) {
		...
		if strings.HasPrefix(ctx.GetContentType(), "application/json") {
			...
			if e := ctx.ParseForm(); e != nil {
				log.Error(ctx, "[/admin.permission/list_project_node] parse form failed", log.CError(e))
				ctx.Abort(cerror.ErrReq)
				return
			}
			// req.ProjectId
			if forms := ctx.GetForms("project_id"); len(forms) > 0 {
				...
				for _, form := range forms {
					if num, e := strconv.ParseUint(form, 10, 32); e != nil {
						log.Error(ctx, "[/admin.permission/list_project_node] data format wrong", log.String("field", "project_id"))
						ctx.Abort(cerror.ErrReq)
						return
					} else {
						req.ProjectId = append(req.ProjectId, uint32(num))
